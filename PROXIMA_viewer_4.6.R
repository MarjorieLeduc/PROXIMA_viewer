library(shiny)
library(DT)
library(shinydashboard)
library(shinyWidgets)
library(shinyjs)
library(ggplot2)
library(plotly)
library(magrittr)
library(purrr)
library(RColorBrewer)
library(imputeLCMD)
library(ggforce)
library(ggrepel)
library(ComplexHeatmap)
library(svglite)
library(ragg)
library(stringr)
library(rstudioapi)

####UI----
options(shiny.maxRequestSize=100*1024^2)

MyList<-list("Sample","Mean of condition","")
names(MyList)<-c("Sample","Mean of condition","")

MyList2<-list("Sample","Mean of condition","Total mean","")
names(MyList2)<-c("Sample","Mean of condition","Total mean","")

ui <- dashboardPage(
  dashboardHeader(title = "Proxima"),
  dashboardSidebar( width = 130,
                    sidebarMenu(id = "tabs",
                                menuItem("Import data", tabName = "ImportData"),
                                menuItem("Descriptive Plots", tabName = "DescPlot"),
                                menuItem("Statistical Plots", tabName = "StatPlot"),
                                menuItem("Protein Plots",tabName="ProtPlot")
                    )
  ),
  dashboardBody(
    tabItems(
      ##### Read data ----
      tabItem(tabName = "ImportData",
              h1("1. Import Data."),
              actionButton("MyFile","Browse...",width="100%"),
              textInput("MyPath",label="rds file :",value="",,width="100%"),
              textOutput("TextNbProt"),
              br(),
              h1("2. Clic 'Apply colors' to display plots."),
              actionButton("ApColors","Apply colors",width="100%"),
              br(),
              fluidRow(
                box(title = "Condition colors",
                    width = 6,
                    uiOutput("colors")),
                box(title = "Replicate colors",
                    width = 6,
                    uiOutput("Repcolors")
                )
              ),  
              DTOutput("value"),
              DTOutput("Tablefdata")
              
      ),
      #####Descriptiveplot ----
      tabItem(tabName = "DescPlot",
              tabBox( width = 0,
                      ###### NbProt ----
                      tabPanel("Nbprot",
                               plotlyOutput('nbprot',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "NbProtformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthNbProt", "Width of downloaded plot (cm)", 20),
                               numericInput("HeightNbProt", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadNbProtplot", "Download")
                      ),
                      ###### PCA ----
                      tabPanel("PCA",  
                               selectInput(
                                 inputId = "TypeDataPCA",
                                 label = "Type of data used to generate PCA",
                                 choices = c("Z-score(intensity)", "Log2(intensity)"),
                                 selected = "Z-score(intensity)"
                               ),
                               selectInput(
                                 inputId = "FilterType",
                                 label = "Type of filter used to generate PCA",
                                 choices = c("% of Valid value in at least one group", "% of Valid value in each group","% of Valid value in total"),
                                 selected = "% of Valid value in at least on group"
                               ),
                               numericInput("PcFilter","% of valid values used to generate PCA",70),
                               selectInput(
                                 inputId = "PCAcolorBy",
                                 label = "Color PCA by ",
                                 choices = c("Condition", "Replicate"),
                                 selected = "Condition"
                               ),
                               checkboxInput(
                                 inputId = "ShowSampleName",
                                 label = "Show sample names",
                                 value = TRUE
                               ),
                               actionButton("DrawPCA","Draw PCA"),
                               helpText("WARRNING : PCA take a long time to be displayed."),
                               textOutput("NbProtPCA"),
                               plotOutput('PCA',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "PCAformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthPCA", "Width of downloaded plot (cm)", 26),
                               numericInput("HeightPCA", "Height of downloaded plot (cm)", 17),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadPCAplot", "Download"),
                               
                               plotlyOutput('PCAprotInteractive',width = "100%", height = "400"),
                               selectizeInput(
                                 inputId = "GenePCA",
                                 label = "Select which genes to show",
                                 choices = NULL,
                                 multiple = TRUE
                               ),
                               numericInput("ForcePCA", "Force to avoid overlap of gene names", 1),
                               plotOutput('PCAprot',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "PCAprotformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthPCAprot", "Width of downloaded plot (cm)", 26),
                               numericInput("HeightPCAprot", "Height of downloaded plot (cm)", 17),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadPCAprotplot", "Download")
                      ),
                      
                      ###### Pearson Correlations ----
                      tabPanel("Pearson correlations", 
                               checkboxInput(
                                 inputId = "PearsonCluster",
                                 label = "Add Sample clustering",
                                 value = FALSE
                               ),
                               plotOutput('Pearson',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "Pearsonformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthPearson", "Width of downloaded plot (cm)", 20),
                               numericInput("HeightPearson", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadPearsonplot", "Download"),
                               br(),
                               plotOutput('PearsonBox',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "PearsonBoxformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthPearsonBox", "Width of downloaded plot (cm)", 20),
                               numericInput("HeightPearsonBox", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadPearsonBoxplot", "Download")
                      ),
                      
                      ###### Euclidean distances ----
                      tabPanel("Euclidean distances", 
                               checkboxInput(
                                 inputId = "EuclidCluster",
                                 label = "Add Sample clustering",
                                 value = FALSE
                               ),
                               plotOutput('Euclid',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "Euclidformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthEuclid", "Width of downloaded plot (cm)", 20),
                               numericInput("HeightEuclid", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadEuclidplot", "Download"),
                               br(),
                               plotOutput('EuclidBox',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "EuclidBoxformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthEuclidBox", "Width of downloaded plot (cm)", 20),
                               numericInput("HeightEuclidBox", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadEuclidBoxplot", "Download")
                      ),
                      ###### Heatmap Log2(intensity) ----
                      tabPanel("Heatmap Log2(intensity)", 
                               colourpicker::colourInput(
                                 inputId = "HighColor",
                                 label = "Choose the color of high expression proteins",
                                 value = "#000000"
                               ),
                               colourpicker::colourInput(
                                 inputId = "MiddleColor",
                                 label = "Choose the color of middle expression proteins",
                                 value = "#056660"
                               ),
                               colourpicker::colourInput(
                                 inputId = "LowColor",
                                 label = "Choose the color of low expression proteins",
                                 value = "#7AFFC3"
                               ),
                               checkboxInput(
                                 inputId = "SampleCluster",
                                 label = "Add Sample clustering",
                                 value = FALSE
                               ),
                               plotOutput('HeatmapLog2',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "HeatmapLog2format",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthHeatmapLog2", "Width of downloaded plot (cm)", 15),
                               numericInput("HeightHeatmapLog2", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadHeatmapLog2plot", "Download")
                      ),
                      
                      ###### Scatterplot ----
                      tabPanel("Scatter plot", 
                               selectInput(
                                 inputId = "ScatterType",
                                 label = "Type of Scatterplot",
                                 choices = MyList,
                                 selected = ""
                               ),
                               selectInput(
                                 inputId = "Sample1",
                                 label = "Sample on X axis",
                                 choices = NULL,
                                 selected = NULL
                               ),
                               selectInput(
                                 inputId = "Sample2",
                                 label = "Sample on Y axis",
                                 choices = NULL,
                                 selected = NULL
                               ),
                               textOutput('CorScatterplot'),
                               actionButton("DrawScatterPlot","Draw Scatterplot"),
                               plotlyOutput('Scatterplotinteractive',width = "100%", height = "400"),
                               selectizeInput(
                                 inputId = "GenesScatterplot",
                                 label = "Select genes to show",
                                 choices = NULL,
                                 multiple = TRUE
                               ),
                               numericInput("ForceScatterplot", "Force to avoid overlap of gene names", 1),
                               plotOutput('Scatterplot',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "ScatterPlotformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthScatterPlot", "Width of downloaded plot (cm)", 15),
                               numericInput("HeightScatterPlot", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadScatterPlot", "Download")
                      ),
                      
                      ###### Rankplot ----
                      tabPanel("Rank plot", 
                               selectInput(
                                 inputId = "RankType",
                                 label = "Type of rank plot",
                                 choices = MyList2,
                                 selected = ""
                               ),
                               selectInput(
                                 inputId = "Rank",
                                 label = "",
                                 choices = NULL,
                                 selected = NULL
                               ),
                               plotlyOutput('Rankinteractive',width = "100%", height = "400"),
                               selectizeInput(
                                 inputId = "GenesRankplot",
                                 label = "Select genes to show",
                                 choices = NULL,
                                 multiple = TRUE
                               ),
                               numericInput("ForceRankplot", "Force to avoid overlap of gene names", 100),
                               plotOutput('Rankplot',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "RankPlotformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthRankPlot", "Width of downloaded plot (cm)", 15),
                               numericInput("HeightRankPlot", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadRankPlot", "Download")
                      )
              )
      ),
      ######Statisticalplot ----
      tabItem(tabName = "StatPlot",
              selectInput(
                inputId = "ComparisonSelect",
                label = "Select the comparison",
                choices = NULL,
                selected = NULL
              ),
              selectInput(
                inputId = "ThresholdType",
                label = "Select the type of threshold",
                choices = c("Pvalue","Qvalue"),
                selected = "Pvalue"
              ),
              numericInput("SignifThreshold", "Significant threshold value", 0.05),
              numericInput("FCThreshold", "Fold change threshold value (1 means no differences)", 1),
              textOutput("NbProtSignif"),
              tabBox( width = 0,
                      
                      ###### VolcanoPlot ----
                      tabPanel("Volcanoplot",
                               fluidRow(
                                 box(title = "Colors",
                                     width = 4,
                                     colourpicker::colourInput(
                                       inputId = "UpColor",
                                       label = "Choose the color of up regulated proteins",
                                       value = "#FF4040"
                                     ),
                                     colourpicker::colourInput(
                                       inputId = "DownColor",
                                       label = "Choose the color of down regulated proteins",
                                       value = "#4C4CFF"
                                     ),
                                     colourpicker::colourInput(
                                       inputId = "NSColor",
                                       label = "Choose the color of non significant proteins",
                                       value = "#C4C4C4"
                                     )    
                                 ),
                                 box(title="Line of threshold",
                                     width=4,
                                     checkboxInput(
                                       inputId = "ShowHline",
                                       label = "Add P/Qvalue threashhold line",
                                       value = FALSE
                                     ),
                                     checkboxInput(
                                       inputId = "ShowVline",
                                       label = "Add Log2(FC) threashhold line",
                                       value = FALSE
                                     ),
                                     selectInput(
                                       inputId = "LineType",
                                       label = "Select the type of the line",
                                       choices = c( "solid", "dashed", "dotted", "dotdash", "longdash", "twodash", "1F", "F1", "4C88C488", "12345678"),
                                       selected = "solid"
                                     ),
                                     colourpicker::colourInput(
                                       inputId = "LineColor",
                                       label = "Choose the color of the line",
                                       value = "#000000"
                                     )
                                 )
                               ),
                               plotlyOutput('IntercativeVolcanoPlot',width = "100%", height = "400"),
                               fluidRow(
                                 box(title="Gene names",
                                     width=4,
                                     selectizeInput(
                                       inputId = "sel_gene_nm",
                                       label = "Select which gene names to show",
                                       choices = NULL,
                                       multiple = TRUE
                                     ),
                                     numericInput("Force", "Force to avoid overlap of gene names", 1),
                                 ),
                                 box(title="Gene points",
                                     width=4,
                                     selectizeInput(
                                       inputId = "GenePoints",
                                       label = "Select which gene points to show",
                                       choices = NULL,
                                       multiple = TRUE
                                     ),
                                     numericInput("PointSize", "Points size", 1),
                                     colourpicker::colourInput(
                                       inputId = "PointColor",
                                       label = "Points Color",
                                       value = "#000000"
                                     )
                                     
                                 )
                               ),
                               plotOutput('VolcanoPlot',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "Volcanoformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthVolcano", "Width of downloaded plot (cm)", 15),
                               numericInput("HeightVolcano", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadVolcanoplot", "Download"),
                               
                               
                      ),
                      ###### Heatmap Significant----
                      tabPanel("Heatmap",
                               colourpicker::colourInput(
                                 inputId = "HighColorSignif",
                                 label = "Choose the color of high expression proteins",
                                 value = "#FF4040"
                               ),
                               colourpicker::colourInput(
                                 inputId = "MiddleColorSignif",
                                 label = "Choose the color of middle expression proteins",
                                 value = "#FFFFFF"
                               ),
                               colourpicker::colourInput(
                                 inputId = "LowColorSignif",
                                 label = "Choose the color of low expression proteins",
                                 value = "#4C4CFF"
                               ),
                               checkboxInput(
                                 inputId = "ShowApDisapeared",
                                 label = "Show Appeared/Disappeared proteins",
                                 value = FALSE
                               ),
                               checkboxInput(
                                 inputId = "ShowGeneName",
                                 label = "Show gene names",
                                 value = FALSE
                               ),
                               plotOutput('HMSignif',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "HMSignifformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthHMSignif", "Width of downloaded plot (cm)", 15),
                               numericInput("HeightHMSignif", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadHMSignif", "Download")
                      ),
                      ###### Heatmap Anova----
                      tabPanel("Anova",
                               colourpicker::colourInput(
                                 inputId = "HighColorSignifAnova",
                                 label = "Choose the color of high expression proteins",
                                 value = "#FF4040"
                               ),
                               colourpicker::colourInput(
                                 inputId = "MiddleColorSignifAnova",
                                 label = "Choose the color of middle expression proteins",
                                 value = "#FFFFFF"
                               ),
                               colourpicker::colourInput(
                                 inputId = "LowColorSignifAnova",
                                 label = "Choose the color of low expression proteins",
                                 value = "#4C4CFF"
                               ),
                               checkboxInput(
                                 inputId = "ShowApDisapearedANOVA",
                                 label = "Show Appeared/Disappeared proteins",
                                 value = FALSE
                               ),
                               checkboxInput(
                                 inputId = "ShowGeneNameAnova",
                                 label = "Show gene names",
                                 value = FALSE
                               ),
                               textOutput("NbProtSignifAnova"),
                               plotOutput('HMAnova',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "HMAnovaformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthHMAnova", "Width of downloaded plot (cm)", 15),
                               numericInput("HeightHMAnova", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadHMAnova", "Download")
                      ),
                      ###### FC Scatterplot ----
                      tabPanel("FC Scatter plot", 
                               selectInput(
                                 inputId = "FC1",
                                 label = "Sample on X axis",
                                 choices = NULL,
                                 selected = NULL
                               ),
                               selectInput(
                                 inputId = "FC2",
                                 label = "Sample on Y axis",
                                 choices = NULL,
                                 selected = NULL
                               ),
                               textOutput('CorFCScatterplot'),
                               actionButton("DrawFCScatterPlot","Draw Scatterplot"),
                               plotlyOutput('FCScatterplotinteractive',width = "100%", height = "400"),
                               selectizeInput(
                                 inputId = "GenesFCScatterplot",
                                 label = "Select genes to show",
                                 choices = NULL,
                                 multiple = TRUE
                               ),
                               numericInput("ForceFCScatterplot", "Force to avoid overlap of gene names", 1),
                               plotOutput('FCScatterplot',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "FCScatterPlotformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthFCScatterPlot", "Width of downloaded plot (cm)", 15),
                               numericInput("HeightFCScatterPlot", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadFCScatterPlot", "Download")
                      ),
                      
                      ###### MAPlot ----
                      tabPanel("MAplot",
                               fluidRow(
                                 box(title = "Colors",
                                     width = 4,
                                     colourpicker::colourInput(
                                       inputId = "UpColorMA",
                                       label = "Choose the color of up regulated proteins",
                                       value = "#FF4040"
                                     ),
                                     colourpicker::colourInput(
                                       inputId = "DownColorMA",
                                       label = "Choose the color of down regulated proteins",
                                       value = "#4C4CFF"
                                     ),
                                     colourpicker::colourInput(
                                       inputId = "NSColorMA",
                                       label = "Choose the color of non significant proteins",
                                       value = "#C4C4C4"
                                     )    
                                 ),
                                 
                                 box(title = "Lines of threshold",
                                     width = 4,
                                     checkboxInput(
                                       inputId = "ShowHlineMA",
                                       label = "Add Log2(FC) threshold line",
                                       value = FALSE
                                     ),
                                     selectInput(
                                       inputId = "LineTypeMA",
                                       label = "Select the type of the line",
                                       choices = c( "solid", "dashed", "dotted", "dotdash", "longdash", "twodash", "1F", "F1", "4C88C488", "12345678"),
                                       selected = "solid"
                                     ),
                                     colourpicker::colourInput(
                                       inputId = "LineColorMA",
                                       label = "Choose the color of the line",
                                       value = "#000000"
                                     )
                                 )
                               ),
                               plotlyOutput('IntercativeMAPlot',width = "100%", height = "400"),
                               fluidRow(
                                 box(title="Gene names",
                                     width=4,
                                     selectizeInput(
                                       inputId = "GenesMAplot",
                                       label = "Select which gene names to show",
                                       choices = NULL,
                                       multiple = TRUE
                                     ),
                                     numericInput("ForceMA", "Force to avoid overlap of gene names", 1)
                                 ),
                                 box(title="Gene points",
                                     width=4,
                                     selectizeInput(
                                       inputId = "GenePointsMA",
                                       label = "Select which gene points to show",
                                       choices = NULL,
                                       multiple = TRUE
                                     ),
                                     numericInput("PointSizeMA", "Points size", 1),
                                     colourpicker::colourInput(
                                       inputId = "PointColorMA",
                                       label = "Points Color",
                                       value = "#000000"
                                     )
                                     
                                 )
                               ),
                               plotOutput('MAPlot',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "MAformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthMA", "Width of downloaded plot (cm)", 25),
                               numericInput("HeightMA", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadMAplot", "Download")
                      )
              )
      ),
      tabItem("ProtPlot",
              selectInput(
                inputId = "ProtPlot_GeneName",
                label = "Select the Gene to show",
                choices = NULL,
                selected = NULL
              ),
              tabBox( width = 0,
                      
                      ###### ProtPlot ----
                      tabPanel("Protein level",
                               textInput("ytitle", "y axis title", "Intensity"),
                               selectInput(
                                 inputId = "BarOrLine",
                                 label = "Select the type of plot display",
                                 choices = c("Bar plot", "Line plot","Paired line plot"),
                                 selected = "Bar plot"
                               ),
                               plotlyOutput('ProtBarplot',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "ProtBarPlotformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthProtBarPlot", "Width of downloaded plot (cm)", 10),
                               numericInput("HeightProtBarPlot", "Height of downloaded plot (cm)", 10),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadProtBarPlot", "Download"),
                               h5(HTML("<font color='red'>Export of all protein plots takes a long time (depending on the number of protein it can take 1h).</font>")),
                               actionButton("ExportAllProtPlots","Export all protein plots"),
                               progressBar(id = "pb3", value = 0,title = "Process 0/0 proteins")
                      ),
                      ###### PepPlot ----
                      tabPanel("Peptide level",
                               plotlyOutput('PepLinePlot',width = "100%", height = "400"),
                               selectInput(
                                 inputId = "PepLinePlotformat",
                                 label = "Format of downloaded plot",
                                 choices = c("svg", "png", "pdf"),
                                 selected = "png"
                               ),
                               numericInput("WidthPepLinePlot", "Width of downloaded plot (cm)", 30),
                               numericInput("HeightPepLinePlot", "Height of downloaded plot (cm)", 15),
                               helpText("Plot is downloaded at 600dpi."),
                               downloadButton("downloadPepLinePlot", "Download")
                      )
              )
      )
    )
  )
)


####Server ----
server <- function(input, output, session) {
  
  
  
  MyFile <-eventReactive(input$MyFile,{
    MyPath<-selectFile(caption="Select .rds file.",label="OK")
    
    # observeEvent(input$dataFile$datapath, {
    out <- readRDS(MyPath)
    out<-out[["fdata"]]
    colnames(out)[which(colnames(out)=="Protein.Group"|colnames(out)=="Protein IDs")]<-"ProteinGroup"
    out$Gene_ProteinGroup<-paste(out$Genes,out$ProteinGroup)
    
    updateSelectizeInput(
      inputId = "sel_gene_nm",
      choices = out$Gene_ProteinGroup,
      server = TRUE,
      selected = NULL
    )
    updateSelectizeInput(
      inputId = "GenePoints",
      choices = out$Gene_ProteinGroup,
      server = TRUE,
      selected = NULL
    )
    updateSelectizeInput(
      inputId = "ProtPlot_GeneName",
      choices = out$Gene_ProteinGroup,
      server = TRUE,
      selected = NULL
    )
    updateSelectizeInput(
      inputId = "GenePCA",
      choices = out$Gene_ProteinGroup,
      server = TRUE,
      selected = NULL
    )
    updateSelectizeInput(
      inputId = "GenesScatterplot",
      choices = out$Gene_ProteinGroup,
      server = TRUE,
      selected = NULL
    )
    
    updateSelectizeInput(
      inputId = "GenesRankplot",
      choices = out$Gene_ProteinGroup,
      server = TRUE,
      selected = NULL
    )
    updateSelectizeInput(
      inputId = "GenesFCScatterplot",
      choices = out$Gene_ProteinGroup,
      server = TRUE,
      selected = NULL
    )
    
    updateSelectizeInput(
      inputId = "GenesMAplot",
      choices = out$Gene_ProteinGroup,
      server = TRUE,
      selected = NULL
    )
    
    updateSelectizeInput(
      inputId = "GenePointsMA",
      choices = out$Gene_ProteinGroup,
      server = TRUE,
      selected = NULL
    )
    
    x<-str_extract(string=MyPath,
                   pattern=".*_([PQ])val(.*)pc(_FC(.*))?.rds",
                   group=c(1,2,4))
    
    updateSelectInput(
      inputId = "ThresholdType",
      selected = paste0(x[1],"value")
    )
    
    updateNumericInput(
      inputId = "SignifThreshold",
      value = as.numeric(x[2])/100
    )
    
    updateNumericInput(
      inputId = "FCThreshold",
      value = if(!is.na(x[3])){as.numeric(x[3])}else{1}
    )
    
    out <- readRDS(MyPath)
    out<-out[["fdata"]]
    outComp<-colnames(out)[str_detect(colnames(out),"Significant")]
    outComp<-gsub("Significant ","",outComp)
    out<-as.list(outComp)
    names(out)<-outComp
    
    updateSelectInput(
      inputId = "ComparisonSelect",
      choices = out
    )
    updateSelectInput(inputId="FC1",choices=out)
    updateSelectInput(inputId="FC2",choices=out)
    
    
    MyPath
  })
  
  observeEvent(input$MyFile,{
    updateTextInput(session, "MyPath", 
                    value = MyFile())
  })
  
  
  
  output$value <- renderDataTable({
    out <- readRDS(input$MyPath)
    datatable(out[["edata"]],
              options = list(pageLength=5, scrollX='400px'))
  })
  
  edata<-reactive({
    out <- readRDS(input$MyPath)
    out[["edata"]]
  })
  
  
  output$TextNbProt<-renderText({
    paste(as.character(nrow(edata())),"proteins,",as.character(ncol(edata())),"samples.")
  })
  
  
  fdata<-reactive({
    out <- readRDS(input$MyPath)
    out<-out[["fdata"]]
    colnames(out)[which(colnames(out)=="Protein.Group"|colnames(out)=="Protein IDs")]<-"ProteinGroup"
    out$Gene_ProteinGroup<-paste(out$Genes,out$ProteinGroup)
    out
  })
  
  output$Tablefdata <- renderDataTable({
    out <- readRDS(input$MyPath)
    out<-out[["fdata"]]
    colnames(out)[which(colnames(out)=="Protein.Group"|colnames(out)=="Protein IDs")]<-"ProteinGroup"
    out$Gene_ProteinGroup<-paste(out$Genes,out$ProteinGroup)
    datatable(out,
              options = list(pageLength=5, scrollX='400px'))
    
    
  })
  
  MyCond<-reactive({
    out <- readRDS(input$MyPath)
    out<-unique(out[["phenodata"]]$Condition)
    out
  })
  
  MyRep<-reactive({
    out <- readRDS(input$MyPath)
    out<-unique(out[["phenodata"]]$Replicate)
    out
  })
  
  phenodata<-reactive({
    out <- readRDS(input$MyPath)
    out[["phenodata"]]
  })
  
  Pepdata<-reactive({
    out <- readRDS(input$MyPath)
    out <- out[["Pepdata"]]
    # out[,3:ncol(out)]<-log2(out[,3:ncol(out)])
    out
  })
  
  #####Functions  ----  
  
  Density<-function(x,y,points){
    #CalcDensityOnGrid
    MyData<-data.frame(x=x,y=y)
    MyData<-MyData[!is.na(MyData[,1])&!is.na(MyData[,2]),]
    
    xmin<-min(MyData[,1])
    xmax<-max(MyData[,1])
    ymin<-min(MyData[,2])
    ymax<-max(MyData[,2])
    
    #GetValuesOnGrid
    xvals<-MyData[,1]
    xStep<-(xmax - xmin) / points
    yvals<-MyData[,2]
    yStep<-(ymax - ymin) / points
    
    n = length(xvals)
    
    #CalcCovariance
    CalcCovariance<-function(data){
      n = nrow(data)
      p = ncol(data)
      
      means<-apply(data,2,mean)
      
      cov = matrix(0,nrow=p,ncol=p)
      for (i in 1:p){
        for (j in 1:i){
          cov[i, j] = sum((data[, i] - means[i]) * (data[, j] - means[j]))
          cov[i, j] = cov[i, j]/ n
          cov[j, i] = cov[i, j]
        }
      }
      return(cov)
    }
    
    cov<-CalcCovariance(MyData)#OK
    
    fact = n^(1 / 6)#OK
    hinv = fact/sqrt(abs(cov))
    hinv[c(1,2), 1] <-hinv[c(1,2),1]*xStep#OK
    hinv[c(1,2), 2] <-hinv[c(1,2),2]*yStep#OK
    
    dx = as.integer(1.0 / hinv[1, 1] * 5)#OK
    dy = as.integer(1.0 / hinv[2, 2] * 5)#OK
    
    values = matrix(0,ncol=points,nrow = points)
    
    xind = as.integer(floor((xvals - xmin) / xStep))#OK
    yind = as.integer(floor((yvals - ymin) / yStep))#OK
    
    for (i in 1:n){
      
      ii <-max(xind[i] - dx, 1): min(xind[i] + dx, points)
      jj <-max(yind[i] - dy, 1): min(yind[i] + dy, points)
      
      #MatrixTimesVector & StandardGaussian
      a1<-((hinv[1,1]*(ii - xind[i]))+(hinv[2,1]*(ii - xind[i])))^2
      a1<-matrix(rep(a1,length(jj)),nrow=length(ii))
      
      a2<-((hinv[1,2]*(jj - yind[i]))+(hinv[2,2]*(jj - yind[i])))^2
      a2<-matrix(rep(a2,each=length(ii)),ncol=length(jj))
      
      MySum<-a1+a2
      StandardGaussian<-exp(-0.5*MySum)/(2*pi)#OK
      
      values[ii, jj] <-values[ii, jj]+StandardGaussian
      
    }
    values<-as.data.frame(values)
    values[values == "NaN"]<-0 
    
    values<-values/max(values)#OK
    xmat <- seq(xmin, xmax, length.out = points)
    ymat <- seq(ymin, ymax, length.out = points)
    
    dvals = rep(0,length(x))
    for (i in 1:length(x)){
      xx = x[i]
      yy = y[i]
      if (!is.na(xx) && !is.na(yy)){
        xind = length(xmat[xmat<=xx])
        yind = length(ymat[ymat<=yy])
        dvals[i] = values[xind, yind]
      } else{
        dvals[i] = NA
      }
    }
    return(dvals)
  }
  
  DataScatterPlot<-function(x,y,edata,fdata){
    MyData<-data.frame(x=x,y=y)
    MyData$Density<-Density(x,y,300)
    MyData$edataRowNames<-row.names(edata)
    MyData<-merge(MyData,fdata,by.x="edataRowNames",by.y="RowNamesfdata",all.x=TRUE)
    return(MyData)
  }
  
  DrawScatterplot<-function(MyData,TitleX,TitleY){
    
    hmcol<- c(colorRampPalette(c("green","yellow"))(5),
              colorRampPalette(c("yellow","orange"))(5),
              colorRampPalette(c("orange","red"))(15),
              colorRampPalette(c("red","blue"))(25),
              colorRampPalette(c("blue","cyan"))(50))
    
    ggplot(data=MyData,aes_string(x=colnames(MyData)[2],y=colnames(MyData)[3],color="Density",label="Gene_ProteinGroup"))+
      geom_point(size=0.5)+
      scale_color_gradientn(colours = hmcol)+
      geom_abline(slope=1,intercept = 0,colour ="white",linewidth=1)+
      theme(panel.background = element_rect(fill = "black"),
            panel.grid.major=element_line(colour="grey40"),
            panel.grid.minor=element_line(colour="grey40"))+
      xlab(TitleX) +
      ylab(TitleY)
  }
  
  #####Descriptive plots  ----
  ######Choose Colors per condition----
  output$colors <- renderUI({
    req(MyCond())
    purrr::map2(
      MyCond(),
      colorRampPalette(brewer.pal(9, "Set1"))(if(length(MyCond())>9){length(MyCond())}else{9})[1:length(MyCond())],
      ~ colourpicker::colourInput(
        inputId = session$ns(.x),
        paste("Choose the color of : ", .x),
        value = .y
      )
    )
  })
  
  output$Repcolors <- renderUI({
    req(MyRep())
    purrr::map2(
      MyRep(),
      colorRampPalette(brewer.pal(8, "Set2"))(if(length(MyRep())>8){length(MyRep())}else{8})[1:length(MyRep())],
      ~ colourpicker::colourInput(
        inputId = session$ns(.x),
        paste("Choose the color of replicate : ", .x),
        value = .y
      )
    )
  })
  
  
  phenodata2<-eventReactive(input$ApColors,{
    color_by_level <- magrittr::set_names(
      purrr::map_chr(
        MyCond(),
        ~ input[[.x]] %||% ""
      ),MyCond())
    
    phenodata2<-phenodata()
    for(i in 1:length(MyCond())){
      phenodata2[phenodata2$Condition==MyCond()[i],"CondColor"]<- color_by_level[i]
    }
    
    color_by_level <- magrittr::set_names(
      purrr::map_chr(
        MyRep(),
        ~ input[[.x]] %||% ""
      ),MyRep())
    
    for(i in 1:length(MyRep())){
      phenodata2[phenodata2$Replicate==MyRep()[i],"RepColor"]<- color_by_level[i]
    }
    
    phenodata2
  })
  
  ######NbProt----
  
  GraphNbProt<-reactive({
    req(edata(),phenodata(),MyCond())
    
    NbProt <- colSums(!is.na(edata()))
    
    x <- data.frame(Sample=colnames(edata()),
                    NbProt=NbProt,
                    Condition=phenodata2()$Condition,
                    CondColor=phenodata2()$CondColor)
    
    Conditions<-factor(x$Condition,levels=unique(x$Condition))
    MyColor<-unique(x$CondColor)
    MySample<-factor(x$Sample,levels=x$Sample)
    
    ggplot(x, aes(x=MySample, y=NbProt, color=Conditions,fill=Conditions)) +
      geom_bar(stat="identity")+
      ylim(0,NA)+
      scale_colour_manual(values = MyColor)+
      scale_fill_manual(values = MyColor)+
      theme_light()+
      theme(axis.text.x = element_text(angle = 90,hjust=1,vjust=0.25),
            axis.title.x = element_blank())+
      ggtitle("Number of Protein per sample")
    
  }) 
  
  output$nbprot<-renderPlotly({ GraphNbProt() })
  
  output$downloadNbProtplot <-downloadHandler(
    function() {
      filename = paste0("NbProt.", input$NbProtformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$NbProtformat,
             plot=GraphNbProt(),
             dpi=600,
             width=input$WidthNbProt,
             height=input$HeightNbProt,
             units="cm")
    })
  
  
  ######PCA----
  
  DataToShow <-eventReactive(input$DrawPCA,{
    req(edata(),phenodata())
    
    FilterGroup <- function(data, MinPc,type=c("1Group","EachGroup","Total")) {
      ValidProt<-rep(NA,nrow(data))
      
      
      if(type=="Total"){
        ValidProt<-100*rowSums(!is.na(edata()))/ncol(edata())
        ValidProt<-ValidProt>=MinPc
      }else{
        for(i in 1:nrow(data)){
          nbVV <- tapply(as.numeric(data[i,]),
                         INDEX=phenodata()[,"Condition"],
                         FUN=complete.cases)#show TRUE FALSE Valid Values for each sample per group
          nbVV <- lapply(nbVV,sum)#count number of Valid Values per group
          
          for(condition in levels(factor(phenodata()[,"Condition"]))) {
            nbVV[[condition]] <- nbVV[[condition]]/table(phenodata()[,"Condition"])[[condition]]*100#%Valid Values per condition.
          }
          
          if (type=="1Group"){
            ValidProt[i] <- any(nbVV >= MinPc)#return TRUE if there is at least one group with 70% of valid values.
          }else if(type=="EachGroup"){
            ValidProt[i] <- !any(nbVV < MinPc)
          }
        }
      }
      return(ValidProt)
    }
    
    if(input$FilterType=="% of Valid value in at least one group"){
      MyType<-"1Group"
    } else if (input$FilterType=="% of Valid value in each group"){
      MyType<-"EachGroup"
    } else if (input$FilterType=="% of Valid value in total"){
      MyType<-"Total"
    }
    
    edata()[FilterGroup(edata(), MinPc = input$PcFilter,type=MyType),]
    
  })
  
  output$NbProtPCA<-renderText({
    paste(as.character(nrow(DataToShow())),"/",as.character(nrow(edata())),"proteins used for PCA.")
  })
  
  my.prc <-eventReactive(input$DrawPCA,{
    req(edata(),phenodata(),DataToShow())
    
    DataToShow <- impute.MinProb(DataToShow(),q = 0.01,tune.sigma =1)
    
    if(input$TypeDataPCA=="Z-score(intensity)"){
      x<-colnames(DataToShow)
      DataToShow <-t(apply(DataToShow,1,scale))
      colnames(DataToShow)<-x  
      prcomp(t(DataToShow) , center=F, scale=F)
    }else if (input$TypeDataPCA=="Log2(intensity)"){
      prcomp(t(DataToShow) , center=T, scale=F)
    }
    
    
  })
  
  PCATitle <-eventReactive(input$DrawPCA,{
    req(edata(),phenodata(),DataToShow())
    paste("PCA", input$TypeDataPCA ,"of proteins with", input$PcFilter,input$FilterType)
  })
  
  GraphPCA<-reactive({
    req(edata(),phenodata(),MyCond())
    
    #PC1 versus PC2
    var_explained <- round(my.prc()$sdev^2/sum(my.prc()$sdev^2)*100,1)
    
    if(input$PCAcolorBy=="Condition"){
      Conditions<-as.factor(phenodata2()$Condition)
      MyColors<-levels(Conditions)
      MyColors<-match(MyColors,phenodata2()$Condition)
      MyColors<-phenodata2()$CondColor[MyColors]
    }else{
      Replicates<-as.factor(phenodata2()$Replicate)
      MyColors<-levels(Replicates)
      MyColors<-match(MyColors,phenodata2()$Replicate)
      MyColors<-phenodata2()$RepColor[MyColors]
    }
    
    x<-as.data.frame(my.prc()$x ) 
    x$Sample<-rownames(x)
    
    if(input$PCAcolorBy=="Condition"){
      g<-ggplot(x,aes(x=PC1,y=PC2,color=Conditions)) +
        geom_mark_hull(aes(group = Conditions,fill = Conditions), alpha = 0.1, radius=unit(5,"mm")) 
    }else{
      g<-ggplot(x,aes(x=PC1,y=PC2,color=Replicates)) +
        geom_mark_hull(aes(group = Replicates, fill = Replicates), alpha = 0.1, radius=unit(5,"mm"))
    }   
    
    g<-g+
      scale_colour_manual(values = MyColors)+
      scale_fill_manual(values = MyColors)+
      geom_point(size=4, from_theme = TRUE) +
      theme_bw(base_size=17) +
      labs(x=paste0("PC1: ",var_explained[1],"%"),
           y=paste0("PC2: ",var_explained[2],"%")) +
      theme(legend.position="right")+
      ggtitle(PCATitle())
    
    if(input$ShowSampleName==TRUE){
      g<-g+geom_text_repel(data = x,
                           aes(label = Sample),
                           nudge_y = 2,size=5,
                           segment.size  = 0.5,
                           force=1,
                           max.overlaps = Inf)
    }
    
    g
  }) 
  
  output$PCA<-renderPlot({ GraphPCA() })
  
  output$downloadPCAplot <-downloadHandler(
    function() {
      filename = paste0("PCA.", input$PCAformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$PCAformat,
             plot=GraphPCA(),
             dpi=600,
             width=input$WidthPCA,
             height=input$HeightPCA,
             units="cm")
    })
  
  
  
  GraphPCAprot<-reactive({
    req(my.prc(), fdata())
    
    x<-as.data.frame(my.prc()$rotation ) 
    x$ProtNumber<-rownames(x)
    y<-fdata()[,c("Gene_ProteinGroup","Genes","RowNamesfdata")]
    x<-merge(x,y,by.x="ProtNumber",by.y="RowNamesfdata",all.x=TRUE, all.y=FALSE)
    
    ggplot(x,aes(x=PC1,y=PC2,label=Gene_ProteinGroup)) +
      geom_point(color="grey")+
      geom_text_repel(data = x[match(input$GenePCA,x$Gene_ProteinGroup),],
                      aes(label = Genes),
                      segment.size  = 0.5,
                      force=input$ForcePCA,
                      color = "black",
                      max.overlaps = Inf)+
      theme_bw()
  }) 
  
  output$PCAprotInteractive<-renderPlotly({ GraphPCAprot() })
  
  output$PCAprot<-renderPlot({ GraphPCAprot() })
  
  output$downloadPCAprotplot <-downloadHandler(
    function() {
      filename = paste0("PCAprot.", input$PCAprotformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$PCAprotformat,
             plot=GraphPCAprot(),
             dpi=600,
             width=input$WidthPCAprot,
             height=input$HeightPCAprot,
             units="cm")
    })
  
  
  ######Pearson correlations----
  ####### Heatmap ----
  GraphPearson<-reactive({
    req(edata(),phenodata(),MyCond())
    
    MyConditions<-data.frame(Conditions=phenodata2()$Condition,row.names=colnames(edata()))
    
    MycolorCond<-phenodata2()[,c(2,3)]
    MycolorCond<-distinct(MycolorCond)
    x<-MycolorCond[,2]
    names(x) <- MycolorCond[,1]
    MycolorCond<-list(x)
    names(MycolorCond)<-"Conditions"
    
    matDist <- cor(edata(),use = "pairwise.complete.obs")
    matDist[matDist == 1]<-NA 
    
    hmcol<- c(colorRampPalette(c("red","sienna1"))(250),
              colorRampPalette(c("sienna1","white"))(150),
              colorRampPalette(brewer.pal(9, 'GnBu'))(100))
    
    MyBreaks<-c(seq(from=0,to=0.4999,length.out=250),
                seq(from=0.5,to=0.7999,length.out=150),
                seq(from=0.8,to=1,length.out=100))
    
    ComplexHeatmap::pheatmap(matDist,  
                             color=hmcol,
                             breaks=MyBreaks,
                             border_color = NA,
                             cluster_rows=input$PearsonCluster,
                             cluster_cols = input$PearsonCluster,
                             legend_breaks=c(0, 0.5, 0.8, 0.9, 1),
                             annotation_col = MyConditions,
                             annotation_row = MyConditions,
                             annotation_colors=MycolorCond,
                             annotation_names_col = FALSE,
                             annotation_names_row =  FALSE,
                             show_rownames=F,
                             show_colnames=T,
                             main = "Pearson's correlation between log2(Intensity)of proteins",
                             na_col = "#000000",
                             fontsize = 13,
                             fontsize_col = 13,
                             angle_col = "90"
    )
  }) 
  
  
  output$Pearson<-renderPlot({ 
    GraphPearson() 
  })
  
  output$downloadPearsonplot <-downloadHandler(
    filename = function() {
      paste0("PearsonHeatmap.", input$Pearsonformat)
    },
    content = function(file) {
      if (input$Pearsonformat == "png") {
        ragg::agg_png(file,
                      res = 600,
                      width=input$WidthPearson,
                      height=input$HeightPearson, 
                      units = "cm"
        )
      } else if (input$Pearsonformat == "pdf") {
        pdf(file,
            width=input$WidthPearson*0.393701,
            height=input$HeightPearson*0.393701)
      } else if (input$Pearsonformat == "svg") {
        svglite::svglite(file,
                         width=input$WidthPearson/2,
                         height =input$HeightPearson/2)
      }
      ComplexHeatmap::draw(GraphPearson())
      dev.off()
    }
  )
  
  
  ####### Boxplot ----
  GraphPearsonBox<-reactive({
    req(edata(),phenodata(),MyCond())
    matDist <- cor(edata(),use = "pairwise.complete.obs")
    matDist[matDist == 1]<-NA
    matDist<-as.data.frame(matDist)
    matDist$Sample1<-row.names(matDist)
    matDist<-reshape(matDist,
                     idvar="Sample1",
                     varying = colnames(matDist)[1:(ncol(matDist)-1)],
                     times= colnames(matDist)[1:(ncol(matDist)-1)], 
                     direction = "long",
                     v.name="Pearson")
    matDist<-matDist[is.na(matDist$Pearson)==FALSE,]
    colnames(matDist)[2]<-"Sample2"
    matDist<-merge(x=matDist,
                   y=phenodata2(),
                   by.x="Sample1",
                   by.y="ShortName",
                   all.x=TRUE,
                   all.y=FALSE)
    colnames(matDist)[4:5]<-paste0(colnames(matDist)[4:5],"1")
    
    matDist<-merge(x=matDist,
                   y=phenodata2(),
                   by.x="Sample2",
                   by.y="ShortName",
                   all.x=TRUE,
                   all.y=FALSE)
    colnames(matDist)[8:9]<-paste0(colnames(matDist)[8:9],"2")
    
    for(i in 1:nrow(matDist)){
      matDist$Cond1_Cond2[i]<-paste(sort(c(matDist$Condition1[i],matDist$Condition2[i])),collapse = "/")
    }
    MyColor<-rep("lightgray",length(levels(factor(matDist$Cond1_Cond2))))
    
    MyNames<-boxplot(data=matDist,Pearson~Cond1_Cond2)$names
    MyNames<-str_split(MyNames,pattern="/")
    for(i in 1:length(MyNames)){
      if(MyNames[[i]][1]==MyNames[[i]][2]){
        #Find color condition
        x<-matDist[matDist$Condition1==MyNames[[i]][1],]$CondColor1
        MyColor[i]<-x
      }
    }
    
    matDist$Cond1_Cond2 <- as.factor(matDist$Cond1_Cond2)
    
    ggplot(matDist, aes(x=Cond1_Cond2, y=Pearson,fill=Cond1_Cond2)) + 
      geom_boxplot(outlier.shape = NA)+
      scale_fill_manual(values=MyColor)+
      geom_jitter(shape=16, position=position_jitter(0.2),size=0.5)+
      theme_light()+
      theme(axis.text.x = element_text(angle = 90,hjust=1),
            axis.title.x = element_blank())+
      theme(legend.position="none")+
      ggtitle("Boxplot of Pearson's correlation between samples") 
    
  }) 
  
  output$PearsonBox<-renderPlot({ GraphPearsonBox() })
  
  output$downloadPearsonBoxplot <-downloadHandler(
    function() {
      filename = paste0("PearsonBox.", input$PearsonBoxformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$PearsonBoxformat,
             plot=GraphPearsonBox(),
             dpi=600,
             width=input$WidthPearsonBox,
             height=input$HeightPearsonBox,
             units="cm")
    })
  
  ######Euclidean distances ----
  #######Heatmap ----
  GraphEuclid<-reactive({
    req(edata(),phenodata(),MyCond())
    
    MyConditions<-data.frame(Conditions=phenodata2()$Condition,row.names=colnames(edata()))
    
    MycolorCond<-phenodata2()[,c(2,3)]
    MycolorCond<-distinct(MycolorCond)
    x<-MycolorCond[,2]
    names(x) <- MycolorCond[,1]
    MycolorCond<-list(x)
    names(MycolorCond)<-"Conditions"
    
    matDist <- as.matrix(dist(t(edata())))
    matDist[matDist == 0]<-NA 
    
    hmcol<- rev(colorRampPalette(brewer.pal(9, 'GnBu'))(100))
    
    ComplexHeatmap::pheatmap(matDist,  
                             color=hmcol,
                             border_color = NA,
                             cluster_rows=input$EuclidCluster,
                             cluster_cols = input$EuclidCluster,
                             annotation_col = MyConditions,
                             annotation_row = MyConditions,
                             annotation_colors=MycolorCond,
                             annotation_names_col = FALSE,
                             annotation_names_row =  FALSE,
                             show_rownames=F,
                             show_colnames=T,
                             main = "Euclidean distance between log2(Intensity)of proteins",
                             na_col = "#000000",
                             fontsize = 13,
                             fontsize_col = 13,
                             angle_col = "90"
    )
  }) 
  
  
  output$Euclid<-renderPlot({ GraphEuclid() })
  
  output$downloadEuclidplot <-downloadHandler(
    filename = function() {
      paste0("EuclideanHeatmap.", input$Euclidformat)
    },
    content = function(file) {
      if (input$Euclidformat == "png") {
        ragg::agg_png(file,
                      res = 600,
                      width=input$WidthEuclid,
                      height=input$HeightEuclid, 
                      units = "cm"
        )
      } else if (input$Euclidformat == "pdf") {
        pdf(file,
            width=input$WidthEuclid*0.393701,
            height=input$HeightEuclid*0.393701)
      } else if (input$Euclidformat == "svg") {
        svglite::svglite(file,
                         width=input$WidthEuclid/2,
                         height =input$HeightEuclid/2)
      }
      ComplexHeatmap::draw(GraphEuclid())
      dev.off()
    }
  )
  
  
  #######BoxPlot ----
  GraphEuclidBox<-reactive({
    req(edata(),phenodata(),MyCond())
    matDist <- as.matrix(dist(t(edata())))
    matDist[matDist == 0]<-NA 
    matDist<-as.data.frame(matDist)
    matDist$Sample1<-row.names(matDist)
    matDist<-reshape(matDist,
                     idvar="Sample1",
                     varying = colnames(matDist)[1:(ncol(matDist)-1)],
                     times= colnames(matDist)[1:(ncol(matDist)-1)], 
                     direction = "long",
                     v.name="Euclidean.distance")
    matDist<-matDist[is.na(matDist$Euclidean.distance)==FALSE,]
    colnames(matDist)[2]<-"Sample2"
    matDist<-merge(x=matDist,
                   y=phenodata2(),
                   by.x="Sample1",
                   by.y="ShortName",
                   all.x=TRUE,
                   all.y=FALSE)
    colnames(matDist)[4:5]<-paste0(colnames(matDist)[4:5],"1")
    
    matDist<-merge(x=matDist,
                   y=phenodata2(),
                   by.x="Sample2",
                   by.y="ShortName",
                   all.x=TRUE,
                   all.y=FALSE)
    colnames(matDist)[8:9]<-paste0(colnames(matDist)[8:9],"2")
    
    for(i in 1:nrow(matDist)){
      matDist$Cond1_Cond2[i]<-paste(sort(c(matDist$Condition1[i],matDist$Condition2[i])),collapse = "/")
    }
    
    MyColor<-rep("lightgray",length(levels(factor(matDist$Cond1_Cond2))))
    
    MyNames<-boxplot(data=matDist,Euclidean.distance~Cond1_Cond2)$names
    MyNames<-str_split(MyNames,pattern="/")
    for(i in 1:length(MyNames)){
      if(MyNames[[i]][1]==MyNames[[i]][2]){
        #Find color of the condition
        MyColor[i]<-matDist[matDist$Condition1==MyNames[[i]][1],]$CondColor1
      }
    }
    
    matDist$Cond1_Cond2 <- as.factor(matDist$Cond1_Cond2)
    
    ggplot(matDist, aes(x=Cond1_Cond2, y=Euclidean.distance,fill=Cond1_Cond2)) + 
      geom_boxplot(outlier.shape = NA)+
      scale_fill_manual(values=MyColor)+
      geom_jitter(shape=16, position=position_jitter(0.2),size=0.5)+
      theme_light()+
      theme(axis.text.x = element_text(angle = 90,hjust=1),
            axis.title.x = element_blank())+
      theme(legend.position="none")+
      ggtitle("Boxplot of Euclidean distance between samples") 
    
    
  }) 
  
  output$EuclidBox<-renderPlot({ 
    GraphEuclidBox() 
  })
  
  output$downloadEuclidBoxplot <-downloadHandler(
    function() {
      filename = paste0("EuclideanBox.", input$EuclidBoxformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$EuclidBoxformat,
             plot=GraphEuclidBox(),
             dpi=600,
             width=input$WidthEuclidBox,
             height=input$HeightEuclidBox,
             units="cm")
    })
  
  
  ######Heatmap Log2 ----
  GraphLog2<-reactive({
    req(edata(),phenodata(),MyCond())
    
    # Select proteins with at least 1 valid value in total
    x <- edata()[rowSums(is.na(edata())) != ncol(edata()), ]
    
    # Mean intensity
    x$MeanLFQ<-apply(x,1,function(x){mean(x,na.rm = TRUE)})
    
    # Sort proteins by mean intensity
    x<-x[order(x$MeanLFQ,decreasing = TRUE),]
    
    # Delete mean columns
    x<-x %>% select(-'MeanLFQ')
    
    #select 2000 prot if nbprot>2000
    nbprot<-nrow(x)
    if (nbprot>2000){
      step<-ceiling(nbprot/2000)
      x<-x[seq(from=1, to=nbprot,by=step ),]
    }
    
    hmcol<- colorRampPalette(c(input$LowColor,input$MiddleColor,input$HighColor))(100)
    
    MyConditions<-data.frame(Conditions=phenodata2()$Condition,row.names=colnames(edata()))
    
    MycolorCond<-phenodata2()[,c(2,3)]
    MycolorCond<-distinct(MycolorCond)
    y<-MycolorCond[,2]
    names(y) <- MycolorCond[,1]
    MycolorCond<-list(y)
    names(MycolorCond)<-"Conditions"
    
    
    #heatmap with sample clustering
    ComplexHeatmap::pheatmap(x,
                             cluster_rows=FALSE,
                             cluster_cols = input$SampleCluster,
                             clustering_distance_cols = "euclidean",
                             clustering_method = "average",
                             col=hmcol,
                             annotation_col = MyConditions,
                             annotation_colors = MycolorCond,
                             na_col = "grey",
                             show_rownames=F,
                             show_colnames=T,
                             annotation_names_col = FALSE,
                             annotation_names_row =  FALSE,
                             main = "Heatmap of Log2(Intensity)",
                             fontsize = 8,
                             fontsize_col = 8,
                             border_color = NA,
                             angle_col="90")
    
  }) 
  
  
  output$HeatmapLog2<-renderPlot({ GraphLog2() })
  
  
  output$downloadHeatmapLog2plot  <-downloadHandler(
    filename = function() {
      paste0("HeatmapLog2.", input$HeatmapLog2format)
    },
    content = function(file) {
      if (input$HeatmapLog2format == "png") {
        ragg::agg_png(file,
                      res = 600,
                      width=input$WidthHeatmapLog2,
                      height=input$HeightHeatmapLog2, 
                      units = "cm"
        )
      } else if (input$HeatmapLog2format == "pdf") {
        pdf(file,
            width=input$WidthHeatmapLog2*0.393701,
            height=input$HeightHeatmapLog2*0.393701)
      } else if (input$HeatmapLog2format == "svg") {
        svglite::svglite(file,
                         width=input$WidthHeatmapLog2/2,
                         height =input$HeightHeatmapLog2/2)
      }
      ComplexHeatmap::draw(GraphLog2())
      dev.off()
    }
  )
  
  
  ######Scatterplot ----
  observeEvent(input$ScatterType,{
    req(edata(),phenodata())
    if (input$ScatterType=="Sample"){
      Label1<-"Sample on the X axis"
      Label2<-"Sample on the Y axis"
      
      x<-as.list(colnames(edata()))
      names(x)<-colnames(edata())
      Value<-x
      
    }else{
      Label1<-"Condition on the X axis"
      Label2<-"Condition on the Y axis"
      
      x<-as.list(unique(phenodata()$Condition))
      names(x)<-unique(phenodata()$Condition)
      Value<-x
    }
    
    updateSelectInput(session, "Sample1",
                      label=Label1,
                      choices = Value,
                      selected=Value[[1]])
    
    updateSelectInput(session, "Sample2",
                      label=Label2,
                      choices = Value,
                      selected=Value[[2]])
    
  },ignoreInit = TRUE)
  
  
  output$CorScatterplot<-renderText({
    if(input$ScatterType=="Sample"){
      x<-edata()[,which(colnames(edata())==input$Sample1)]
      y<-edata()[,which(colnames(edata())==input$Sample2)]
    }else{
      x<-edata()[,which(phenodata()$Condition == input$Sample1)]
      x<-apply(x,MARGIN=1,mean, na.rm = TRUE)
      y<-edata()[,which(phenodata()$Condition == input$Sample2)]
      y<-apply(y,MARGIN=1,mean, na.rm = TRUE)
    }
    
    paste("r =",round(cor(x,y,use="pairwise.complete.obs"),5))
  })
  
  
  DensityScatter <-eventReactive(input$DrawScatterPlot,{
    req(edata(),fdata(),phenodata())
    
    if(input$ScatterType=="Sample"){
      x<-edata()[,which(colnames(edata())==input$Sample1)]
      y<-edata()[,which(colnames(edata())==input$Sample2)]
    }else{
      x<-edata()[,which(phenodata()$Condition == input$Sample1)]
      x<-apply(x,MARGIN=1,mean, na.rm = TRUE)
      y<-edata()[,which(phenodata()$Condition == input$Sample2)]
      y<-apply(y,MARGIN=1,mean, na.rm = TRUE)
    }
    
    DataScatterPlot(x,y,edata(),fdata())
    
  })
  
  DrawScatter<-eventReactive(input$DrawScatterPlot,{
    req(DensityScatter())
    DrawScatterplot(DensityScatter(),TitleX=input$Sample1,TitleY=input$Sample2)
    
  })
  
  ScatterPlot<-reactive({
    req(DrawScatter())
    
    g<-DrawScatter()
    g+geom_text_repel(data = DensityScatter()[match(input$GenesScatterplot,DensityScatter()$Gene_ProteinGroup),],
                      aes(label = Genes),
                      segment.size  = 0.5,
                      force=input$ForceScatterplot,
                      color = "white",
                      max.overlaps = Inf)
    
    
  })
  
  output$Scatterplotinteractive<-renderPlotly({ ScatterPlot() })
  
  output$Scatterplot<-renderPlot({ ScatterPlot() })
  
  output$downloadScatterPlot <-downloadHandler(
    function() {
      filename = paste0("ScatterPlot_",input$Sample1,"_",input$Sample2,".", input$ScatterPlotformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$ScatterPlotformat,
             plot=ScatterPlot(),
             dpi=600,
             width=input$WidthScatterPlot,
             height=input$HeightScatterPlot,
             units="cm")
    })
  
  
  ######Rankplot ----
  
  observeEvent(input$RankType,{
    req(edata(),phenodata())
    if (input$RankType=="Sample"){
      Label1<-"Choose the sample to show :"
      x<-as.list(colnames(edata()))
      names(x)<-colnames(edata())
      Value<-x
      
    }else if(input$RankType=="Mean of condition"){
      Label1<-"Choose the condition to show :"
      x<-as.list(unique(phenodata()$Condition))
      names(x)<-unique(phenodata()$Condition)
      Value<-x
      
    }else{
      Label1<-"Total mean to show :"
      x<-list("Total mean")
      names(x)<-"Total mean"
      Value<-x
    }
    
    updateSelectInput(session, "Rank",
                      label=Label1,
                      choices = Value,
                      selected=Value[[1]])
    
  },ignoreInit = TRUE)
  
  RankPlot<-reactive({
    
    if(input$RankType=="Sample"){
      x<-edata()[,which(colnames(edata())==input$Rank)]
    }else if(input$RankType=="Mean of condition"){
      x<-edata()[,which(phenodata()$Condition == input$Rank)]
      x<-as.data.frame(x)
      x<-apply(x,MARGIN=1,mean, na.rm = TRUE)
    }else{
      x<-apply(edata(),MARGIN=1,mean, na.rm = TRUE)
    }
    
    MyData<-data.frame(Log2Intensity=x,
                       Rank=length(x)-rank(x,na.last = FALSE),
                       edataRowNames=row.names(edata()))
    MyData<-merge(MyData,fdata(),by.x="edataRowNames",by.y="RowNamesfdata",all.x=TRUE)
    
    ggplot(MyData,aes(x=Rank,y=Log2Intensity,label=Gene_ProteinGroup))+
      geom_point(color="grey")+
      geom_point(data = MyData[match(input$GenesRankplot,MyData$Gene_ProteinGroup),],
                 color="black",
                 size=2)+
      geom_text_repel(data = MyData[match(input$GenesRankplot,MyData$Gene_ProteinGroup),],
                      aes(label = Genes),
                      segment.size  = 0.5,
                      force=input$ForceRankplot,
                      color = "black",
                      max.overlaps = Inf)+
      ylab(paste("Log2(",input$Rank,"Intensity)"))+
      theme_bw()
    
  })
  
  
  output$Rankinteractive<-renderPlotly({ RankPlot() })
  
  output$Rankplot<-renderPlot({ RankPlot() })
  
  output$downloadRankPlot <-downloadHandler(
    function() {
      filename = paste0("RankPlot_",input$Rank,".", input$RankPlotformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$RankPlotformat,
             plot=RankPlot(),
             dpi=600,
             width=input$WidthRankPlot,
             height=input$HeightRankPlot,
             units="cm")
    })
  
  
  
  #####Statistical plots  ----
  
  output$NbProtSignif<-renderText({
    req(MyListeProt(),MyColNumber())
    
    Pval<-max(fdata()[which(MyListeProt()==TRUE),MyColNumber()],na.rm=TRUE)
    Qval<-max(fdata()[which(MyListeProt()==TRUE),(MyColNumber()+1)],na.rm=TRUE)
    paste(as.character(length(which(MyListeProt()==TRUE))),"/",as.character(length(MyListeProt())),"significant proteins.","\nPvalue=",Pval,",FDR=",Qval,".")
  })
  
  ######Volcanoplot ----
  
  GraphVolcano<-reactive({
    req(fdata())
    
    n<-if(input$ThresholdType=="Pvalue"){0}else{1}
    
    x<-fdata()[is.na(fdata()[,MyColNumber()])==FALSE,]
    x<-x[,c(MyColNumber(),MyColNumber()+n,MyColNumber()+2,8,5)]
    x$Type<-input$NSColor
    x[x[,2]<input$SignifThreshold & x[,3]>log2(input$FCThreshold),"Type"]<-input$UpColor
    x[x[,2]<input$SignifThreshold & x[,3]<(-log2(input$FCThreshold)),"Type"]<-input$DownColor
    x[,1]<--log10(x[,1])
    colnames(x)<-gsub(pattern="\\s",replacement="_",x=colnames(x))
    colnames(x)<-gsub(pattern="\\/",replacement="_",x=colnames(x))
    
    g<-ggplot(x,aes_string(x=colnames(x)[3],y=colnames(x)[1],color="Type",label="Gene_ProteinGroup")) +
      geom_point(size=1) +
      scale_colour_manual(values = levels(as.factor(x$Type)))+
      geom_point(data=x[match(input$GenePoints,x$Gene_ProteinGroup),],
                 color=input$PointColor,
                 size=input$PointSize)+
      geom_text_repel(data = x[match(input$sel_gene_nm,x$Gene_ProteinGroup),],
                      aes_string(label = "Genes"),
                      segment.size  = 0.5,
                      force=input$Force,
                      color = "black",
                      max.overlaps = Inf)+
      theme_bw(base_size=10) + 
      labs(x=paste0("Log2(",input$ComparisonSelect,")"),
           y="-log10(Pvalue)") +
      theme(legend.position="none")+
      ggtitle(paste("Volcano plot",input$ComparisonSelect))
    
    if(input$ShowHline==TRUE){
      
      if(input$ThresholdType=="Pvalue"){
        MyThreshold<--log10(input$SignifThreshold)
      }else{
        MyThreshold<-max(x[x[,2]>input$SignifThreshold,1])
      }
      
      
      g<-g+geom_hline(linetype=input$LineType,
                      color=input$LineColor,
                      yintercept=MyThreshold
      )
    }
    
    if(input$ShowVline==TRUE){
      g<-g+geom_vline(linetype=input$LineType,
                      color=input$LineColor,
                      xintercept=log2(input$FCThreshold)
      )+
        geom_vline(linetype=input$LineType,
                   color=input$LineColor,
                   xintercept=-log2(input$FCThreshold)
        )
    }
    g
  })
  
  output$VolcanoPlot<-renderPlot({ GraphVolcano() })
  
  output$IntercativeVolcanoPlot<-renderPlotly({ GraphVolcano() })
  
  output$downloadVolcanoplot <-downloadHandler(
    function() {
      filename = paste0("Volcanoplot_",input$ComparisonSelect,".", input$Volcanoformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$Volcanoformat,
             plot=GraphVolcano(),
             dpi=600,
             width=input$WidthVolcano,
             height=input$HeightVolcano,
             units="cm")
    })
  
  ######Heatmap significant ----
  MyColNumber <-reactive({
    req(fdata())
    MyColNumber<-which(colnames(fdata())==paste("Pvalue",input$ComparisonSelect))
    MyColNumber
  })
  
  MyListeProt <-reactive({
    req(fdata(),MyColNumber())
    n<-if(input$ThresholdType=="Pvalue"){0}else{1}
    
    #filter significative proteins
    if(input$ShowApDisapeared==TRUE){
      MyListeProt<-(fdata()[,(MyColNumber()+n)]<input$SignifThreshold
                    & (fdata()[,(MyColNumber()+2)]<(-log2(input$FCThreshold ))
                       |fdata()[,(MyColNumber()+2)]>log2(input$FCThreshold))
      )|(
        str_detect(string=fdata()[,(MyColNumber()-1)],pattern="peared")==TRUE)
    }else{
      MyListeProt<-(fdata()[,(MyColNumber()+n)]<input$SignifThreshold
                    & (fdata()[,(MyColNumber()+2)]<(-log2(input$FCThreshold ))
                       |fdata()[,(MyColNumber()+2)]>log2(input$FCThreshold)))
    }
    
    MyListeProt
  })
  
  
  GraphHMSignif<-reactive({
    req(fdata(),phenodata2(),MyListeProt(),edata())
    
    #select columns of the comparison
    MyListX<-which(phenodata2()$Condition == str_split(input$ComparisonSelect,"/")[[1]][1])
    MyListY<-which(phenodata2()$Condition == str_split(input$ComparisonSelect,"/")[[1]][2])
    
    #filter significative proteins
    Genes<-fdata()[which(MyListeProt()==TRUE),c(5,2)]#"Genes","Protein.Group"
    Genes[is.na(Genes$Genes),"Genes"]<-Genes[is.na(Genes$Genes),2]#"Protein.Group"
    Genes<-Genes$Genes
    
    MyListeProt<-fdata()[which(MyListeProt()==TRUE),"RowNamesfdata"]
    MyListeProt<-data.frame(x1=MyListeProt,x2=MyListeProt)
    
    DataToShow<-cbind(edata(),RowNamesedata=row.names(edata()))
    DataToShow<-merge(DataToShow,MyListeProt,by.x="RowNamesedata",by.y="x1",all=FALSE)
    DataToShow<-DataToShow %>% select(-c('RowNamesedata','x2'))
    
    DataToShow<-DataToShow[,c(MyListX,MyListY)]
    
    if(nrow(MyListeProt)>1){  
      #Z-score
      MyZscore<-t(apply(DataToShow,1,scale))
      colnames(MyZscore)<-colnames(DataToShow)
      rownames(MyZscore)<-Genes
      
      hmcol<- colorRampPalette(c(input$LowColorSignif,input$MiddleColorSignif,input$HighColorSignif))(100)
      hmcol<-c(input$LowColorSignif,hmcol,input$HighColorSignif)
      
      MyBreaks<-seq(from=-2,to=2,length.out=100)
      MinBreaks<-if(min(MyZscore,na.rm=TRUE)<(-2)){min(MyZscore,na.rm=TRUE)}else{-2.0001}
      MaxBreaks<-if(max(MyZscore,na.rm=TRUE)>2){max(MyZscore,na.rm=TRUE)}else{2.0001}
      MyBreaks<-c(MinBreaks,MyBreaks,MaxBreaks)
      
      MyConditions<-data.frame(Conditions=phenodata2()$Condition[c(MyListX,MyListY)],row.names=colnames(MyZscore))
      
      MycolorCond<-phenodata2()[c(MyListX[1],MyListY[1]),c(2,3)]
      x<-MycolorCond[,2]
      names(x) <- MycolorCond[,1]
      MycolorCond<-list(x)
      names(MycolorCond)<-"Conditions"
      
      # cor_matrix <- cor(MyZscore, use = "pairwise.complete.obs", method = "pearson")
      # dist_matrix <- as.dist(1 - cor_matrix)
      # dist_matrix[is.na(dist_matrix)]<-2
      # hc_cols <- hclust(dist_matrix,method = "complete")
      dist_matrix<-dist(t(MyZscore))
      dist_matrix[is.na(dist_matrix)]<-max(dist_matrix,na.rm=TRUE)
      hc_cols<-hclust(dist_matrix,method="complete")
      
      cor_matrix <- cor(t(MyZscore), use = "pairwise.complete.obs", method = "pearson")
      dist_matrix <- as.dist(1 - cor_matrix)
      dist_matrix[is.na(dist_matrix)]<-2
      hc_rows <- hclust(dist_matrix,method = "complete")
      
      ComplexHeatmap::pheatmap(MyZscore,
                               cluster_rows = hc_rows,
                               cluster_cols = hc_cols,
                               color=hmcol,
                               breaks=MyBreaks,
                               legend_breaks=c(if(MinBreaks==-2.0001){-2}else{c(round(MinBreaks,1),-2)},
                                               if(MaxBreaks==2.0001){2}else{c(2,round(MaxBreaks,1))}),
                               annotation_col = MyConditions,
                               annotation_colors=MycolorCond,
                               na_col = "grey",
                               show_rownames=input$ShowGeneName,
                               show_colnames=TRUE,
                               annotation_names_col = FALSE,
                               annotation_names_row =  FALSE,
                               main = paste0("Heatmap of Zscore of differential proteins ",
                                             input$ComparisonSelect,
                                             " (",input$ThresholdType,input$SignifThreshold,if(input$FCThreshold!=1){paste0(", FC>",input$FCThreshold)}else{""},
                                             ")"
                               ),
                               border_color = NA,
                               angle_col="90"
      )
    }
  })
  
  output$HMSignif<-renderPlot({ GraphHMSignif() })
  
  output$downloadHMSignif <-downloadHandler(
    filename = function() {
      paste0("HeatmapSignificant_",input$ComparisonSelect,".", input$HMSignifformat)
    },
    content = function(file) {
      if (input$HMSignifformat == "png") {
        ragg::agg_png(file,
                      res = 600,
                      width=input$WidthHMSignif,
                      height=input$HeightHMSignif, 
                      units = "cm"
        )
      } else if (input$HMSignifformat == "pdf") {
        pdf(file,
            width=input$WidthHMSignif*0.393701,
            height=input$HeightHMSignif*0.393701)
      } else if (input$HMSignifformat == "svg") {
        svglite::svglite(file,
                         width=input$WidthHMSignif/2,
                         height =input$HeightHMSignif/2)
      }
      ComplexHeatmap::draw(GraphHMSignif())
      dev.off()
    }
  )
  
  ######Heatmap Anova ----
  MyColNumberAnova<-reactive({
    req(fdata())
    MyColNumber<-which(colnames(fdata())=="Anova Pval")
    MyColNumber
  })
  
  MyListeProtAnova <-reactive({
    req(fdata(),MyColNumberAnova())
    n<-if(input$ThresholdType=="Pvalue"){0}else{1}
    
    #filter significative proteins
    if(input$ShowApDisapearedANOVA==TRUE){
      MyListeProt<-(fdata()[,(MyColNumberAnova()+n)]<input$SignifThreshold
      )|(
        str_detect(string=fdata()[,(MyColNumber()-1)],pattern="peared")==TRUE)
    }else{
      MyListeProt<-fdata()[,(MyColNumberAnova()+n)]<input$SignifThreshold
    }
    
    
    MyListeProt
  })
  
  output$NbProtSignifAnova<-renderText({
    req(MyListeProtAnova(),MyColNumberAnova())
    
    Pval<-max(fdata()[which(MyListeProtAnova()==TRUE),MyColNumberAnova()])
    Qval<-max(fdata()[which(MyListeProtAnova()==TRUE),(MyColNumberAnova()+1)])
    paste(as.character(length(which(MyListeProtAnova()==TRUE))),"/",as.character(length(MyListeProtAnova())),"significant proteins.","\nPvalue=",Pval,",FDR=",Qval,".")
  })
  
  GraphHMAnova<-reactive({
    req(fdata(),phenodata2(),edata())
    
    #filter significative proteins
    Genes<-fdata()[which(MyListeProtAnova()==TRUE),c(5,2)]#"Genes","Protein.Group"
    Genes[is.na(Genes$Genes),"Genes"]<-Genes[is.na(Genes$Genes),2]#"Protein.Group"
    Genes<-Genes$Genes
    
    MyListeProt<-fdata()[which(MyListeProtAnova()==TRUE),"RowNamesfdata"]
    MyListeProt<-data.frame(x1=MyListeProt,x2=MyListeProt)
    
    DataToShow<-cbind(edata(),RowNamesedata=row.names(edata()))
    DataToShow<-merge(DataToShow,MyListeProt,by.x="RowNamesedata",by.y="x1",all=FALSE)
    DataToShow<-DataToShow %>% select(-c('RowNamesedata','x2'))
    
    
    if(nrow(MyListeProt)>1){  
      #Z-score
      MyZscore<-t(apply(DataToShow,1,scale))
      
      colnames(MyZscore)<-colnames(DataToShow)
      rownames(MyZscore)<-Genes
      
      hmcol<- colorRampPalette(c(input$LowColorSignifAnova,input$MiddleColorSignifAnova,input$HighColorSignifAnova))(100)
      hmcol<-c(input$LowColorSignifAnova,hmcol,input$HighColorSignifAnova)
      
      MyBreaks<-seq(from=-2,to=2,length.out=100)
      MinBreaks<-if(min(MyZscore,na.rm=TRUE)<(-2)){min(MyZscore,na.rm=TRUE)}else{-2.0001}
      MaxBreaks<-if(max(MyZscore,na.rm=TRUE)>2){max(MyZscore,na.rm=TRUE)}else{2.0001}
      MyBreaks<-c(MinBreaks,MyBreaks,MaxBreaks)
      
      MyConditions<-data.frame(Conditions=phenodata2()$Condition,row.names=colnames(MyZscore))
      
      MycolorCond<-phenodata2()[,c(2,3)]
      x<-MycolorCond[,2]
      names(x) <- MycolorCond[,1]
      MycolorCond<-list(x)
      names(MycolorCond)<-"Conditions"
      
      # cor_matrix <- cor(MyZscore, use = "pairwise.complete.obs", method = "pearson")
      # dist_matrix <- as.dist(1 - cor_matrix)
      # dist_matrix[is.na(dist_matrix)]<-2
      # hc_cols <- hclust(dist_matrix,method = "complete")
      dist_matrix<-dist(t(MyZscore))
      dist_matrix[is.na(dist_matrix)]<-max(dist_matrix,na.rm=TRUE)
      hc_cols<-hclust(dist_matrix,method="complete")
      
      cor_matrix <- cor(t(MyZscore), use = "pairwise.complete.obs", method = "pearson")
      dist_matrix <- as.dist(1 - cor_matrix)
      dist_matrix[is.na(dist_matrix)]<-2
      hc_rows <- hclust(dist_matrix,method = "complete")
      
      
      ComplexHeatmap::pheatmap(MyZscore,
                               cluster_rows = hc_rows,
                               cluster_cols = hc_cols,
                               color=hmcol,
                               breaks=MyBreaks,
                               legend_breaks=c(if(MinBreaks==-2.0001){-2}else{c(round(MinBreaks,1),-2)},
                                               if(MaxBreaks==2.0001){2}else{c(2,round(MaxBreaks,1))}),
                               annotation_col = MyConditions,
                               annotation_colors=MycolorCond,
                               na_col = "grey",
                               show_rownames=input$ShowGeneNameAnova,
                               show_colnames=TRUE,
                               annotation_names_col = FALSE,
                               annotation_names_row =  FALSE,
                               main = paste0("Heatmap of Zscore of Anova differential proteins ",
                                             " (",input$ThresholdType,input$SignifThreshold,")"
                               ),
                               border_color = NA,
                               angle_col="90"
      )
    }
  })
  
  output$HMAnova<-renderPlot({ GraphHMAnova() })
  
  output$downloadHMAnova  <-downloadHandler(
    filename = function() {
      paste0("HeatmapAnova.", input$HMAnovaformat)
    },
    content = function(file) {
      if (input$HMAnovaformat == "png") {
        ragg::agg_png(file,
                      res = 600,
                      width=input$WidthHMAnova,
                      height=input$HeightHMAnova, 
                      units = "cm"
        )
      } else if (input$HMAnovaformat == "pdf") {
        pdf(file,
            width=input$WidthHMAnova*0.393701,
            height=input$HeightHMAnova*0.393701)
      } else if (input$HMAnovaformat == "svg") {
        svglite::svglite(file,
                         width=input$WidthHMAnova/2,
                         height =input$HeightHMAnova/2)
      }
      ComplexHeatmap::draw(GraphHMAnova())
      dev.off()
    }
  )
  
  ######FC Scatterplot ----
  
  
  output$CorFCScatterplot<-renderText({
    
    x<-fdata()[,which(colnames(fdata())==paste("Log2",input$FC1))]
    y<-fdata()[,which(colnames(fdata())==paste("Log2",input$FC2))]
    
    
    paste("r =",round(cor(x,y,use="pairwise.complete.obs"),5))
  })
  
  
  DensityFCScatter <-eventReactive(input$DrawFCScatterPlot,{
    req(edata(),fdata())
    
    x<-fdata()[,which(colnames(fdata())==paste("Log2",input$FC1))]
    y<-fdata()[,which(colnames(fdata())==paste("Log2",input$FC2))]
    
    
    DataScatterPlot(x,y,edata(),fdata())
    
  })
  
  DrawFCScatter<-eventReactive(input$DrawFCScatterPlot,{
    req(DensityFCScatter())
    DrawScatterplot(DensityFCScatter(),TitleX=paste("Log2",input$FC1),TitleY=paste("Log2",input$FC2))
    
  })
  
  FCScatterPlot<-reactive({
    req(DrawFCScatter())
    
    g<-DrawFCScatter()
    g+geom_text_repel(data = DensityFCScatter()[match(input$GenesFCScatterplot,DensityFCScatter()$Gene_ProteinGroup),],
                      aes(label = Genes),
                      segment.size  = 0.5,
                      force=input$ForceFCScatterplot,
                      color = "white",
                      max.overlaps = Inf)+
      geom_vline(xintercept = 0,colour ="white",linewidth=1)+
      geom_hline(yintercept = 0,colour ="white",linewidth=1)
    
    
  })
  
  output$FCScatterplotinteractive<-renderPlotly({ FCScatterPlot() })
  
  output$FCScatterplot<-renderPlot({ FCScatterPlot() })
  
  output$downloadFCScatterPlot <-downloadHandler(
    function() {
      filename = paste0("FCScatterPlot_",input$FC1,"_",input$FC2,".", input$FCScatterPlotformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$FCScatterPlotformat,
             plot=FCScatterPlot(),
             dpi=600,
             width=input$WidthFCScatterPlot,
             height=input$HeightFCScatterPlot,
             units="cm")
    })
  
  
  ######MAplot ----
  
  GraphMA<-reactive({
    req(fdata(),edata())
    
    y<-edata()
    y$MeanLog2Intensity<-apply(y,MARGIN=1,FUN=function(x){mean(x,na.rm=TRUE)})
    y$yRnames<-rownames(y)
    x<-fdata()[is.na(fdata()[,MyColNumber()])==FALSE,]
    x<-x[,c((MyColNumber()+2),8,5)]
    x$Type<-input$NSColorMA
    x[x[,1]>log2(input$FCThreshold),"Type"]<-input$UpColorMA
    x[x[,1]<(-log2(input$FCThreshold)),"Type"]<-input$DownColorMA
    colnames(x)<-gsub(pattern="\\s",replacement="_",x=colnames(x))
    colnames(x)<-gsub(pattern="\\/",replacement="_",x=colnames(x))
    x$xRnames<-rownames(x)
    x<-merge(x,y, by.x="xRnames",by.y="yRnames",all.y=FALSE,all.x=TRUE)
    x<-x[,c(2:5,ncol(x))]
    
    
    g<-ggplot(x,aes_string(x=colnames(x)[5],y=colnames(x)[1],color="Type",label="Gene_ProteinGroup")) +
      geom_point(size=1) +
      scale_colour_manual(values = levels(as.factor(x$Type)))+
      geom_point(data=x[match(input$GenePointsMA,x$Gene_ProteinGroup),],
                 color=input$PointColorMA,
                 size=input$PointSizeMA)+
      geom_text_repel(data = x[match(input$GenesMAplot,x$Gene_ProteinGroup),],
                      aes_string(label = "Genes"),
                      segment.size  = 0.5,
                      force=input$ForceMA,
                      color = "black",
                      max.overlaps = Inf)+
      theme_bw(base_size=10) + 
      labs(x=paste0("Mean Log2(Intensity)"),
           y=paste("Log2(",input$ComparisonSelect,")")) +
      theme(legend.position="none")+
      ggtitle(paste("MA plot",input$ComparisonSelect))
    
    
    if(input$ShowHlineMA==TRUE){
      g<-g+geom_hline(linetype=input$LineTypeMA,
                      color=input$LineColorMA,
                      yintercept=log2(input$FCThreshold)
      )+
        geom_hline(linetype=input$LineTypeMA,
                   color=input$LineColorMA,
                   yintercept=-log2(input$FCThreshold)
        )
    }
    g
  })
  
  
  
  output$MAPlot<-renderPlot({ GraphMA() })
  
  output$IntercativeMAPlot<-renderPlotly({ GraphMA() })
  
  output$downloadMAplot <-downloadHandler(
    function() {
      filename = paste0("MAplot_",input$ComparisonSelect,".", input$MAformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$MAformat,
             plot=GraphMA(),
             dpi=600,
             width=input$WidthMA,
             height=input$HeightMA,
             units="cm")
    })
  
  
  
  #####Proteins plots  ----
  ###### ProtPlot ----
  df2<-reactive({
    Myedata<-2^edata()
    Myedata<-cbind(Myedata,RowNamesedata=row.names(edata()))
    Myfdata<-fdata()[,c("RowNamesfdata","Gene_ProteinGroup")]
    df<-merge(Myfdata,Myedata,by.x="RowNamesfdata",by.y="RowNamesedata")
    df<-df[,-1]
    df<-reshape(data=df,
                idvar="Gene_ProteinGroup",
                varying = colnames(df)[2:ncol(df)],
                times=colnames(df)[2:ncol(df)],
                v.names="Intensity",
                direction="long")
    
    #Ajoute conditions
    df<-merge(df,phenodata2(),by.x="time",by.y="ShortName",all.x=TRUE)
    colnames(df)[1]<-"Sample"
    df<-df[!is.na(df$Intensity),]
    df
  })
  
  showProtPlot<-function(i){
    x<-df2()[df2()$Gene_ProteinGroup==i,]
    
    for(j in MyCond()){
      if(length(which(x$Condition==j))==0){
        y<-data.frame(Sample=NA,
                      Gene_ProteinGroup=i,
                      Intensity=0,
                      Condition=j,
                      CondColor="",
                      Replicate="",
                      RepColor="")
        x<-rbind(x,y)
      }
    }
    
    #Calcul SD et Mean
    x.summary <- x %>%
      group_by(Condition) %>%
      summarise(
        sd = sd(Intensity, na.rm = TRUE),
        Intensity = mean(Intensity)
      )
    x.summary$Sample<-NA
    
    x<-x[x$Intensity>0,]
    
    if(input$BarOrLine=="Paired line plot"){
      MyRep2<-sort(unique(x$Replicate))
      MyColor2<-c()
      RepColor<-distinct(phenodata2()[,c("Replicate","RepColor")])
      for(j in 1:length(MyRep2)){
        #Find color of the replicate
        MyColor2<-c(MyColor2,RepColor[RepColor$Replicate==MyRep2[j],"RepColor"])
      }
      
      ggplot(x, aes(x=Condition, y=Intensity, colour=Replicate)) + 
        geom_point()+
        geom_line(aes(group=Replicate))+
        scale_colour_manual(values = MyColor2)+
        ggtitle(i)+
        ylab(input$ytitle)+
        theme(axis.title.x=element_blank(),
              axis.text.x=element_text(angle=90, hjust=1),
              plot.title = element_text(size=8))+
        theme_light() 
      
    } else {
      g<-ggplot(x, aes(x=Condition, y=Intensity, label=Sample)) +
        geom_errorbar( aes(ymin = Intensity-sd, ymax = Intensity+sd), 
                       data = x.summary, width = 0.3) +
        theme_light()+
        ylab(input$ytitle)+
        theme(axis.title.x=element_blank(),
              axis.text.x=element_text(angle=90, hjust=1),
              plot.title = element_text(size=8))+
        ggtitle(i)
      
      if(input$BarOrLine=="Bar plot"){
        
        MyCond2<-sort(unique(x.summary$Condition))
        MyColor2<-c()
        CondColor<-distinct(phenodata2()[,c("Condition","CondColor")])
        for(i in 1:length(MyCond2)){
          #Find color of the condition
          MyColor2<-c(MyColor2,CondColor[CondColor$Condition==MyCond2[i],"CondColor"])
        }
        
        g<-g+geom_col(data = x.summary, fill = MyColor2, color = "grey30") +
          geom_point(pch=16,size=4,alpha=0.4, color = "black")
      }else{
        g<-g+geom_line(data=x.summary,aes(x=Condition,y=Intensity,group=1))+
          geom_point(data=x.summary,pch=16,size=4)
      }
      g
    }
  }
  
  
  GraphProt<-reactive({
    req(df2(),MyCond())
    i<-input$ProtPlot_GeneName
    showProtPlot(i)
  })
  
  output$ProtBarplot<-renderPlotly({ GraphProt() })
  
  observeEvent(input$ExportAllProtPlots,{
    req(df2(), MyCond())
    disable("ExportAllProtPlots")
    
    if(!file.exists(paste0(dirname(input$MyPath),"/Prot_BarPlot"))){
      dir.create(file.path(dirname(input$MyPath),"Prot_BarPlot"))
    }
    
    df2<-df2()
    df2$Gene_ProteinGroup<-gsub(pattern="\\|",replacement = "_",x=df2$Gene_ProteinGroup)
    df2$Gene_ProteinGroup<-gsub(pattern="\\/",replacement = "_",x=df2$Gene_ProteinGroup)
    df2$Gene_ProteinGroup<-gsub(pattern="\\:",replacement = "_",x=df2$Gene_ProteinGroup)
    
    
    NbTot<-length(unique(df2$Gene_ProteinGroup))
    n<-0
    
    for(i in unique(df2$Gene_ProteinGroup)){
      g<-showProtPlot(i)
      ggsave(file=paste0(dirname(input$MyPath),"/Prot_BarPlot/",i,".png"),
             plot=g,
             width=input$WidthProtBarPlot, height=input$HeightProtBarPlot,
             units="cm",dpi = 600)
      n<-n+1
      updateProgressBar(
        id = "pb3",
        value = n, total = NbTot,
        title = paste0(n,"/",NbTot," protein plots exported in ",dirname(input$MyPath),"/Prot_BarPlot")
      )
    }
    
    enable("ExportAllProtPlots")
    
  })
  
  output$downloadProtBarPlot <-downloadHandler(
    function() {
      filename = paste0(input$ProtPlot_GeneName,".", input$ProtBarPlotformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$ProtBarPlotformat,
             plot=GraphProt(),
             dpi=600,
             width=input$WidthProtBarPlot,
             height=input$HeightProtBarPlot,
             units="cm")
    })
  
  
  
  ###### PeptidePlot ----
  GraphPep<-reactive({
    req(Pepdata())
    i<-input$ProtPlot_GeneName
    x<-Pepdata()[Pepdata()$Gene_ProteinGroup==i,]
    
    x<-reshape(data=x,
               idvar=c("Gene_ProteinGroup","Peptide"),
               varying=colnames(x)[3:ncol(x)],
               times=colnames(x)[3:ncol(x)],
               v.names="Log2Intensity",
               direction="long")
    colnames(x)[3]<-"Sample"
    x$OxyM<-" Unmodified"
    x$OxyM[str_detect(string=x$Precursor.Id,pattern="(UniMod:35)")]<-"OxyM"
    
    #Pepide plot
    g<-ggplot(x,aes(x=Sample,y=Log2Intensity,group=Peptide,color=Peptide))+
      geom_line(aes(linetype=OxyM),linewidth=1.1)+
      geom_point(size=2)+
      theme_light()+
      theme(axis.title.x=element_blank(),
            axis.text.x=element_text(angle=90, hjust=1),
            plot.title = element_text(size=8),
            legend.position = "none")+
      ggtitle(i)
    
    #Add prot data
    Myfdata<-fdata()
    y<-Myfdata[Myfdata$Gene_ProteinGroup==i,"RowNamesfdata"]
    y<-edata()[row.names(edata())==y,]
    y<-as.data.frame(t(y))
    y$Sample<-row.names(y)
    colnames(y)[1]<-"Log2Intensity"
    y$Peptide<-"Protein"
    
    g<-g+geom_line(data=y,aes(x=Sample,y=Log2Intensity),color="black",linewidth=1.2,linetype="dotted")+
      geom_point(data=y,pch=1,size=3,color="black")
    g
    
  })
  
  output$PepLinePlot<-renderPlotly({ GraphPep() })
  
  output$downloadPepLinePlot <-downloadHandler(
    function() {
      filename = paste0(input$ProtPlot_GeneName,"_peptides.", input$PepLinePlotformat)
    },
    content = function(file) {
      ggsave(file,
             device=input$PepLinePlotformat,
             plot=GraphPep(),
             dpi=600,
             width=input$WidthPepLinePlot,
             height=input$HeightPepLinePlot,
             units="cm")
    })
  
}



####Run----
shinyApp(ui, server)